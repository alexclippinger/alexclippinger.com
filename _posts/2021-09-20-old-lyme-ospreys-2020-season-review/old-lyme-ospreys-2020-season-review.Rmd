...
3. Great Island and the Lower Connecticut River Valley area are important ecological and conservation areas.
...
<iframe src="https://www.google.com/maps/d/embed?mid=1Ryu5cu-Xl8g2f10yxbB37S0JmeqWnJGR" width="640" height="480"></iframe>
...
```
In my datasets, there are `r dim(all_nests)[1]` total nests and `r dim(ol_nests)[1]` nests in Old Lyme. This plot shows that 91% of thesenests in the entire state and 96% of nests in Old Lyme were actively monitored in 2020.
...
ggplot(data = all_nest_types, aes(x = count, y = reorder(nest_type, count), fill = nest_type)) +
  geom_col() +
  geom_text(aes(label = count), vjust = 0.5, hjust = -.25, size = 2.5) +
  theme_minimal() +
  theme(
    legend.position = "none",
    axis.title.y = element_blank(),
...
<figure>
  ...
  <figcaption>Map of Missing Nest Type Data in Old Lyme, CT.</figcaption>
</figure>
