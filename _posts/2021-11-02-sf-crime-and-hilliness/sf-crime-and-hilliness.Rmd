...
There is a negative correlation between **elevation** and crime. It also appears that this relationship is not linear, so the model may fit better if log-transformed. Additionally, there is a negative correlation between **slope** and crime also showing signs of a non-linear relationship, supporting the justification for a transformation. Lastly, there appears to be a weak negative correlation, or even possibly no significant relationship, between **median income** and car break-ins.

Although not directly relevant to this regression analysis given that we assume that elevation, slope, and median income are constant over time, Figure 3 demonstrates the trend of break-ins over the temporal range of our dataset (2018-Present).
...
ts_plot + monthly_plot + plot_annotation(title = 'Figure 3. Temporal trend of break-ins',
                                         theme = theme(plot.title = element_text(hjust = 0.5)))
```

An upward trend in crime is observed, especially beginning in 2020 during the COVID-19 pandemic.

Lastly, the figure below visualizes the spatial distribution of the crime data within the city, as well as provides a relative idea of the areas of higher elevation. It can be observed that the crime data is distributed across most of the land area within the study area. Additionally, the areas with significant terrain (such as Twin Peaks, Potrero Hill, and Nob Hill) can be identified by the areas with concentrations of darker green points.
...
The QQ plot indicates that the residuals are relatively normally distributed for values up to approximately 2 on the theoretical normal distribution. This supports the conclusion from Figure 5 that the model is not normally distributed for high values.
...
y1 <- ggplot(data = crime_lm, aes(x = elev, y = res)) +
  geom_point(alpha = 0.25) + theme_classic() + 
...
Next, we can interpret the significance of these coefficients using the p-value. For all independent variables, the p-value is less than 0.001 indicating that each of the coefficients represent a significant effect.

Lastly, the $R^2$ value of approximately 0.09 indicates that ~9% of the variance in number of break-ins is accounted for in the model. This is an expected result, as crime is most likely going to be related to many other socio-economic and demographic factors that were not considered in this analysis. Many variables could have been included, such as population density, racial composition, land use, etc. However, this analysis is only focused on the effects of topography.

### Model 2: Logarithmic transformation

It is also important to compare the results of the OLS analysis with an alternative regression equation. In earlier sections visualizing the simple relationships, it was identified that the relationship of both elevation and slope with break-ins appeared to be non-linear. Therefore, it could be appropriate to transform the equation. The equation and summary below demonstrate the different results when log-transforming the dependent variable.
...
In conclusion, the linear regression model predicts that there are significant negative relationships between the key independent variables (slope and elevation) and the dependent variable (number of car break-ins).

The study found that certain assumptions of OLS were violated, meaning that the OLS method is possibly not the estimator with lowest variance. This should be taken into account while interpreting the resulting coefficients of the variables.

This analysis has room for improvement, including but not limited to:
1. Adding more key socio-economic and demographic variables.
2. Accounting for the OLS assumption violation, either by transforming the data or using another estimator.
3. More thoroughly researching and selecting a proper transformation method.
